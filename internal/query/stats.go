...
// TODO(abdul): Stats function does a full 360 connection loop, where it closes connection from
// pack db and reconnects with bolt package and reconnects back to pack db after the data is received.
// There might be a cleaner way to get the stats collected by bolt package without have

// boltStats prints detailed statistics about the bolt database file.
func (q *Query) Stats(verbose bool) error {
	var (
		p = util.PrettyInt
	)

	// close connection to db because pack.DB already has access to it
	if err := q.db.Close(); err != nil {
		return err
	}

	db, err := bolt.Open(strings.Join([]string{q.dbPath,
		q.activeTable + ".db"}, "/"), 0666, &bolt.Options{
	...
	// close bolt db connection
	if err := db.Close(); err != nil {
		return err
	}

	// reconnect using pack db
	// arbitrarry delay for
	q.db, err = pack.OpenDatabase(q.dbPath, q.activeTable, "*", q.opts)
	if err != nil {
